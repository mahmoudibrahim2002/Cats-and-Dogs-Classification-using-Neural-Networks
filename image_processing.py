...
def process_single_image(img_path, processing_options, output_path=None, is_preview=False):
    ...
    try:
        ...
        # For preview mode, we want to show exactly what the current options would do
        if is_preview:
        ...
        # Add new operations after existing ones
        ...
        if processing_options['transformations']['apply'] and processing_options['transformations']['method'] != "None":
            ...
            # Blur - only apply if enabled and method is not "None"
            ...
            # Sharpening - only apply if enabled and method is not "None"
            ...
            # Edge detection - only apply if enabled and method is not "None"
            ...
            # Morphological operations - only apply if enabled and operation is not "None"
            ...
            # Normalization - only apply if enabled and scheme is not "None"
        ...
        # For actual processing (not preview)
        else:
            processed_img = img.copy()
            
            # Apply all processing steps (same logic as preview but will save the results)
            if processing_options['color_mode'] != "Keep original":
                if processing_options['color_mode'] == "Grayscale":
                    if len(processed_img.shape) == 3:
                        processed_img = cv2.cvtColor(processed_img, cv2.COLOR_BGR2GRAY)
            ...
            # Pet-specific processing
            if processing_options['pet_specific']['fur_enhancement']:
                processed_img = apply_fur_enhancement(
                    processed_img,
                    processing_options['pet_specific']['fur_strength']
                )
                
            if processing_options['pet_specific']['eye_enhancement']:
                processed_img = apply_pet_eye_enhancement(
                    processed_img,
                    processing_options['pet_specific']['eye_strength']
                )
                
            if processing_options['pet_specific']['background_simplify']:
                processed_img = apply_background_simplification(
                    processed_img,
                    processing_options['pet_specific']['bg_method']
                )
...
def basic_processing_page():
    ...
    if len(all_image_paths) > 0:
        ...
        # Update processing_options dictionary to REMOVE pet_specific section
        processing_options = {
...
def main():
    ...
    # Sidebar content
    with st.sidebar:
        st.markdown('<h1 class="sidebar-title">Image Processing</h1>', unsafe_allow_html=True)
